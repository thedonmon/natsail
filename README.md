...
| [`@natsail/effect`](https://www.npmjs.com/package/@natsail/effect)           | Scoped Effect subject and session Streams  | Core, session package, Effect peer                     |
...
| ------------------------------- | ---------- | --------------------------- | ------------------------------------------------------------- |
| `@natsail/effect`               | Prerelease | Effect v4 RC                | Bounded subject Streams, scoped services, and typed failures  |
...
Effect v4 programs can consume Core NATS subjects directly from a scoped, bounded Stream. NATSail continues to own connection policy; Effect owns demand, structured cancellation, and finalization:
...
Each consumer owns one cold subscription that closes after success, failure, or interruption. Subject Streams suspend the awaited Core NATS handler when their bounded buffer is full by default. Applications can instead choose a typed overflow error, dropping, or sliding for best-effort data. Core NATS remains ephemeral and at-most-once; use JetStream for replay and acknowledgement.
...
| Adapter ergonomics           | Effect v4 directly owns bounded Core subject Streams and scoped cleanup. Validated definitions can still share atomic reducer state across Effect, React, and RxJS.    | Effect-native JetStream pull, batch materialization, and explicit-ack processor APIs still need their load proofs.                        |
...
2. Prove Effect v4 subject Streams under slow-consumer load, then add Effect-native JetStream pull, batch materialization, and explicit-ack processor APIs using the same bounded vocabulary.
