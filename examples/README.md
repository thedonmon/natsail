...
The RxJS and Effect examples deliberately use the same six conversations, seeded message counts, interaction design, assistant responder, multi-tab flow, and performance counters. Switch among 8, 48, 96, 240, 1,000, and 5,000-message histories, then use the selectable 40/250/1,000-message busy-room action to compare presentation batching without changing the product workload. The rail separates adapter catch-up from completed React rendering and reports actual commit duration when React's development or profiling build exposes it.
