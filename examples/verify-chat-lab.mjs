...
export async function verifyChatLab({ adapter, baseUrl, devScript, screenshot }) {
  ...
  try {
    ...
    await expect(primary.getByText(body)).toBeVisible()
    await expect(primary.getByText('Another tab sent a message')).toBeVisible()
    await expect(primary.getByText('I’m on it.')).toBeVisible()
    await expect(primary.getByText(/I replayed the conversation context/)).toBeVisible()
